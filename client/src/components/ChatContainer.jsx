    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messagesDummyData.map((message, i) => (
          <div key={i} className={`flex items-end gap-2 justify-end ${message.senderId !== '680f5116f10f3cd28382ed02' && 'flex-row-reverse'}`}>
            {message.image ? (
              <img src={message.image} alt="" className='max-w-[230px] rounded-lg border border-gray-700 overflow-hidden mb-8' />
            ) : (
              <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${message.senderId !== '680f5116f10f3cd28382ed02' ? 'rounded-br-none' : 'rounded-bl-none'}`}>
                {message.text}
              </p>
            )}
